...
{   
    ...
    switch (_type)
    {
        ...
        case iCarouselTypeCoverFlow2:
        {
            ...
            CGFloat y = (clampedOffset * 0.5 * tilt + offset * spacing) * _itemHight;
//            NSLog(@"CGFloat y :%f",y);
#pragma mark----在这里修改
            if (_vertical)
            {
                transform = CATransform3DTranslate(transform, 0.0, x*1.2, z);
                return CATransform3DRotate(transform, -clampedOffset * M_PI_2 * tilt, -1.0, 0.0, 0.0);
            }
            else
            {
                transform = CATransform3DTranslate(transform, x*1.2, 0.0, z);//
...
- (UIView *)containView:(UIView *)view
{
    ...
    if (!_itemWidth)
    {
        ...
        _itemHight = _vertical? view.bounds.size.width : view.bounds.size.height;
    }
